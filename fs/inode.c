...
struct inode_position {
   bool	 two_sec;	// inode是否跨扇区 一个扇区大概放2-3i节点
...
static void inode_locate(struct partition* part, uint32_t inode_no, struct inode_position* inode_pos){
   ...
   uint32_t off_byte_size = inode_no * inode_size;
   inode_pos->sec_lba  = inode_table_lba + off_byte_size / SECTOR_SIZE;
   inode_pos->off_size = off_byte_size % SECTOR_SIZE;
   inode_pos->two_sec = ((inode_pos->off_size + inode_size) > SECTOR_SIZE)? true:false;
}

// 将inode写入到分区part 
void inode_sync(struct partition* part, struct inode* inode, void* io_buf) {	 // io_buf是用于硬盘io的缓冲区
   struct inode_position inode_pos;
   inode_locate(part, inode->i_no, &inode_pos);	  //inode位置信息会存入inode_pos
...
/* 为使通过sys_malloc创建的新inode被所有任务共享, 需要将inode置于内核空间,故需要临时将cur_pbc->pgdir置为NULL */
...
// 1 回收inode占用的所有块 
   ...
   uint32_t all_blocks[140] = {0};	  //12个直接块+128个间接块(512B/4B)
