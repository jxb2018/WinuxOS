...
//在全局的文件表file_table中得到一个空位free_slot，成功返回下标,失败返回-1
int32_t get_free_slot_in_global(void) {
   ...
   if (fd_idx == MAX_FILE_OPEN) { //已经打开了最大文件数
      printk("in get_free_slot_in_global:cannot open more files\n");
      return -1;
   }
   return fd_idx;
}
...
int32_t pcb_fd_install(int32_t globa_fd_idx) {
   ...
   if (local_fd_idx == MAX_FILES_OPEN_PER_PROC) {
      printk("in pcd_fd_install:cannot open more files\n");
      return -1;
   }
   return local_fd_idx;
}

// 分配一个i结点,返回i结点号 ，成功便返回分区i结点位图inode_bitmap中的下标，失败-1
...
// 在分区的块位图block_bitmap中分配1个扇区,返回其扇区地址
